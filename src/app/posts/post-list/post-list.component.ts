...
export class PostListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe((paramMap: ParamMap) => {
      if (paramMap.has("results")) {
        ...
        this.postsService.getSearchResults(this.query, this.postsPerPage, 1);
  ...
  onSortResult(option: any) {
    ...
    if (option === "hot") {
      ...
      this.posts.sort(
        (a, b) =>
          parseInt(a.commentsNumber, 10) - parseInt(b.commentsNumber, 10)
      );
    } else {
      // default
  ...
  onDelete(postId: string) {
    ...
    this.postsService.deletePost(postId).subscribe(
      () => {
        // Fix display bugs
        if (
          this.totalPosts - 1 - this.postsPerPage * (this.currPage - 1) <=
          0
        ) {
          // Set current pg
